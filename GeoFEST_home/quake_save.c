...
/*---------------------------------------------------------------------------*/
  ...
  /*  3rd exploratory slip complete... now decide which way to migrate next...  */
  printf("\nslip delta=%g  applied=%g  slip total= %g  strain energy=%g\n\n",
  slip_delta,(fltgrp_ptr+f_index)->q_amount,total_slip,global.elas_energy) ;
    
      nextslot = 3 ;  /* this is the last search position checked */
    
      while(slip_delta > SLIP_TOL)
       {
           ...
           switch(lastslot)
             {
              ...
              case 2:
			   (fltgrp_ptr+f_index)->q_amount = TWO*slip_delta;
              break ;
        ...
        else if((energy3 > energy2) && (energy2 > energy1))
          {
		   energy3 = energy2;
		   energy2 = energy1;
		   lastslot = nextslot ;
        ...
        else if((energy3 > energy1) && (energy1 > energy2))
          {
		   energy3 = energy2;
		   lastslot = nextslot ;
		   nextslot = 2;
        ...
        else if((energy1 > energy3) && (energy3 > energy2))
          {
		   energy1 = energy2;
		   lastslot = nextslot ;
		   nextslot = 2;
       ...
       /* now execute the next required slip operation... */
       
          total_slip += (fltgrp_ptr+f_index)->q_amount ;
		  elgrp_loop( RHS_QUAKE ) ;  /* amount determined above */
		  clear_real(global.del_displ, loc_sys.numnp*fe_sys.ndof) ;
		  ...
		  if(nextslot == 1)      energy1 = global.elas_energy ;
		  else if(nextslot == 2) energy2 = global.elas_energy ;
		  else if(nextslot == 3) energy3 = global.elas_energy ;
  ...
  slip_delta,(fltgrp_ptr+f_index)->q_amount,total_slip,global.elas_energy) ;
     ...
     printf("Net slip of fault strand %d: %f times programmed\n",f_index+1,total_slip) ;
